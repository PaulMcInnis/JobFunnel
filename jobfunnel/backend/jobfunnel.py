...
class JobFunnel(
    ...
    def __init__(
        ...
        self.config = config
        self.__date_string = date.today().strftime(
            "%Y-%m-%d"
        )
    ...
    def run(
        ...
        if (
            self.config.no_scrape
        ):

            # Load cache since --no-scrape is set
            self.logger.info(
                "Skipping scraping, running with --no-scrape."
            )
            if os.path.exists(
                self.daily_cache_file
            ):
                scraped_jobs_dict = self.load_cache(
                    self.daily_cache_file
                )
            else:
                self.logger.warning(
                    "No incoming jobs, missing cache: %s",
                    self.daily_cache_file,
                )
        else:
        ...
        duplicate_jobs = (
            []
        )  # type: List[DuplicatedJob]
        if (
            self.master_jobs_dict
            and scraped_jobs_dict
        ):
            ...
            for match in duplicate_jobs:
                ...
                if (
                    match.type
                    in [
                        DuplicateType.KEY_ID
                        or DuplicateType.EXISTING_TFIDF
                    ]
                ):
    ...
    def scrape(
        ...
        self.logger.info(
            "Scraping local providers with: %s",
            self.config.scraper_names,
        )
        ...
        for scraper_cls in (
            ...
            try:
                ...
                self.logger.error(
                    f"Failed to scrape jobs for {scraper_cls.__name__}"
                )
        ...
        self.logger.info(
            "Completed all scraping, found %d new jobs.",
            len(
                jobs
            ),
        )
    ...
    def recover(
        ...
        self.logger.info(
            "Recovering jobs from all cache files in cache folder"
        )
        if os.path.exists(
            self.config.user_block_list_file
        ):
    ...
    def load_cache(
        ...
        if not os.path.exists(
            ...
            cache_dict = pickle.load(
                open(
                    cache_file,
                    "rb",
                )
            )
            jobs_dict = cache_dict[
                "jobs_dict"
            ]
            version = cache_dict[
                "version"
        ...
        with open(
            ...
            for row in csv.DictReader(
                ...
                if (
                    ...
                    self.logger.warning(
                        "Unknown status %s, setting to UNKNOWN",
                        status_str,
                    )
                    status = (
                        JobStatus.UNKNOWN
                ...
                if (
                    ...
                    self.logger.warning(
                        "Unknown locale %s, setting to UNKNOWN",
                        locale_str,
                    )
                    locale = (
                        locale.UNKNOWN
    ...
    def update_user_block_list(
        ...
        for job in (
            ...
            if (
                ...
                if (
                    job.key_id
                    not in self.job_filter.user_block_jobs_dict
                ):
                    n_jobs_added += 1
    ...
    def update_duplicates_file(
        ...
        if (
            ...
            if (
                ...
                self.logger.debug(
                    "Extending existing duplicate jobs dict."
                )
                with open(
                    self.config.duplicates_list_file,
                    "w",

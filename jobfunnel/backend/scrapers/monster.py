...
class BaseMonsterScraper(
    ...
    def __init__(
        ...
        if (
            ...
            self.logger.warning(
                "Monster does not support remoteness in query."
            )
    ...
    def set(
        ...
        if (
            ...
            pot_wage_cell = job._raw_scrape_data.find(
                "div",
                attrs={
                    "class": "col-xs-12 cell"
                },
            )
            if pot_wage_cell:
                pot_wage_value = pot_wage_cell.find(
                    "div"
                )
                if pot_wage_value:
                    job.wage = (
                        pot_wage_value.text.strip()
                    )
        elif (
            parameter
            == JobField.DESCRIPTION
            ...
            for li in job._raw_scrape_data.find_all(
                ...
                if (
                    table_key
                    and table_key.text.strip().lower()
    ...
    def get_job_soups_from_search_result_listings(
        ...
        initial_search_results_html = self.session.get(
            search_url
        )
        ...
        n_pages = self._get_num_search_result_pages(
        ...
        job_soups_dict = __get_job_soups_by_key_id(
            initial_search_results_soup
        )
        ...
        if (
            ...
            for page in range(
                ...
                job_soups_dict.update(
                    __get_job_soups_by_key_id(
                        next_listings_page_soup
                    )
                )

        # TODO: would be cool if we could avoid key_id scrape duplication in get
        return list(
            job_soups_dict.values()
        )

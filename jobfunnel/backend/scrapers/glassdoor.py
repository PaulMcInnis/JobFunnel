...
class BaseGlassDoorScraper(
    ...
    def get_job_soups_from_search_result_listings(
        ...
        request_html = self.session.post(
            search_url,
            data=data,
        )
        ...
        job_soup_list = self._parse_job_listings_to_bs4(
            soup_base
        )
    ...
    def _search_page_for_job_soups(
        ...
        """
        self.logger.debug(
            f"Scraping listings page {listings_page_url}"
        )
    ...
    def _get_next_page_url(
        ...
        assert (
            part_url
            is not None
        ), "Unable to find next page in listing soup!"
...
class GlassDoorMetricRadius:
    ...
    def quantize_radius(
        ...
        if (
            ...
            radius = 200
        return GLASSDOOR_RADIUS_MAP[
            radius
        ]


class GlassDoorScraperCANEng(
    GlassDoorMetricRadius,
    BaseGlassDoorScraper,
    BaseCANEngScraper,
):
    """Scrapes jobs from www.glassdoor.ca"""


class GlassDoorScraperUSAEng(
    BaseGlassDoorScraper,
    BaseUSAEngScraper,
):

...
class BaseIndeedScraper(
    ...
    def __init__(
        ...
        if (
            self.config.search_config.remoteness
            == Remoteness.PARTIALLY_REMOTE
        ):
            self.logger.warning(
                "Indeed does not support PARTIALLY_REMOTE jobs"
            )
    ...
    def get_job_soups_from_search_result_listings(
        ...
        pages = self._get_num_search_result_pages(
            search_url
        )
        self.logger.info(
    ...
    def get(
        ...
        if (
            ...
            if remote_field:
                remoteness_str = (
                    remote_field.text.strip().lower()
                )
                if (
                    remoteness_str
                    in REMOTENESS_STR_MAP
                ):
                    return REMOTENESS_STR_MAP[
                        remoteness_str
                    ]
            return (
                Remoteness.UNKNOWN
        ...
        if (
            ...
            return (
                ...
                "limit={}&filter={}{}".format(
                    ...
                    self._quantize_radius(
                        self.config.search_config.radius
                    ),
                    self.max_results_per_page,
                    int(
                        self.config.search_config.return_similar_results
                    ),
                    REMOTENESS_TO_QUERY[
                        self.config.search_config.remoteness
                    ],
            ...
            raise ValueError(
                f"No html method {method} exists"
            )

    def _quantize_radius(
        self,
        radius: int,
    ) -> int:
    ...
    def _get_num_search_result_pages(
        ...
        request_html = self.session.get(
            search_url
        )
        self.logger.debug(
            "Got Base search results page: %s",
            search_url,
        )
        query_resp = BeautifulSoup(
            request_html.text,
            self.config.bs4_parser,
        )
        ...
        if (
            ...
            raise ValueError(
                ...
                " province or state.".format(
                    search_url
                )
            )

        num_res = num_res.contents[
            0
        ].strip()
        num_res = int(
            re.findall(

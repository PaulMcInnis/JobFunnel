...
class JobFilter(
    ...
    def __init__(
        ...
        blocked_company_names_list: Optional[
            List[
                str
            ]
        ] = None,
        max_job_date: Optional[
            datetime
        ] = None,
        ...
        self.blocked_company_names_list = (
            blocked_company_names_list
            or []
        )
    ...
    def filterable(
        ...
        return bool(
            ...
            or (
                job.company
                in self.blocked_company_names_list
            )
            or (
                job.post_date
                and self.max_job_date
                and job.is_old(
                    self.max_job_date
                )
            )
            or (
                ...
                and job.key_id
                in self.user_block_jobs_dict
            )
            or (
                check_existing_duplicates
                and self.is_duplicate(
                    job
                )
            )
            or (
                job.remoteness
                != Remoteness.UNKNOWN
                and self.desired_remoteness
                != Remoteness.ANY
                and job.remoteness
                != self.desired_remoteness
            )
        )
    ...
    def find_duplicates(
        ...
        filt_existing_jobs_dict = deepcopy(
            existing_jobs_dict
        )
        ...
        if (
            len(
                filt_incoming_jobs_dict.keys()
            )
            + len(
                filt_existing_jobs_dict.keys()
            )
    ...
    def tfidf_filter(
        ...
        if existing_jobs_dict:
            self.logger.debug(
                "Running TFIDF on incoming vs existing data."
            )
            (
                reference_ids,
                reference_words,
            ...
            self.logger.debug(
                "Running TFIDF on incoming data only."
            )
            reference_ids = (
                query_ids,
            )
            ...
            if (
                ...
                new_duplicate_jobs_list.append(
                    DuplicatedJob(
                        ...
                        duplicate=filt_incoming_jobs_dict[
                            query_id
                        ],
        ...
        if (
            not new_duplicate_jobs_list
        ):
            self.logger.debug(
                "Found no duplicates by content-matching."
            )

...
def calculate_delays(
    ...
    if (
        delay_config.algorithm
        == DelayAlgorithm.CONSTANT
    ):
        delay_vals = _c_delay(
            list_len,
            delay_config.max_duration,
        )
    elif (
        delay_config.algorithm
        == DelayAlgorithm.LINEAR
    ):
        delay_vals = _lin_delay(
            list_len,
            delay_config.max_duration,
        )
    elif (
        ...
        delay_vals = _sig_delay(
            list_len,
            delay_config.max_duration,
        )
    else:
        raise ValueError(
            f"Cannot calculate delay for {delay_config.algorithm}"
        )

    # Check if minimum delay is above 0 and less than last element
    if (
        delay_config.min_duration
        > 0
    ):
    ...
    if (
        ...
        if (
            delay_config.converge
        ):  # checks if converging delay is True

...
def test_search_config_query_string(
    mocker,
    keywords,
    exp_query_str,
):
...
def test_search_config_init(
    ...
    if not locale:
        ...
        with pytest.raises(
            ValueError,
            match=r"Unknown domain for locale.*",
        ):
            cfg = SearchConfig(

...
INCORRECT_TEST_YAML = os.path.join(
    get_data_path(),
    "../data/incorrect_test_config.yml",
)
...
def test_parse_cli_invalid_args(
    argv,
    exception,
):
    with pytest.raises(
...
def test_build_config_dict_invalid_settings(
    argv,
    exception,
):
    args = parse_cli(
...
def test_build_config_dict_load_args(
    argv,
):
    args = parse_cli(
        argv
...
def test_build_config_dict_inline_args(
    argv,
):
    args = parse_cli(
        argv
